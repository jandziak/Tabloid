...
probka_DIR <- '/home/marta/Documents/erka/probka/'

docs_collection_basectag_DIR  <- paste0(probka_DIR, 'docs_collection_basectag_probka/')
docs_collection_baseform2_DIR <- paste0(probka_DIR, 'docs_collection_baseform2_probka/')
docs_collection_raw_DIR  <- paste0(probka_DIR, 'docs_collection_raw_probka/')
docs_collection_tagged_DIR  <- paste0(probka_DIR, 'docs_collection_tagged_probka/')

metadata_PATH <- paste0(probka_DIR, 'ARTICLES_metadata.dat')
termstats_PATH <- paste0(probka_DIR, 'ARTICLES_termstats.dat')
text_PATH <- paste0(probka_DIR, 'ARTICLES_text.dat')
interpunkcja_PATH <-'/home/marta/Documents/erka/interpunkcja.csv'
sylaby_PATH <-'/home/marta/Documents/erka/sylaby.csv'
stats_tabloid_nietabloid_PATH <-'/home/marta/Documents/erka/sentiment_stats_tabloid_nietabloid.txt'
...
# responses
EXAMPLES_expert_PATH <- '/home/marta/Documents/erka/EXAMPLES_expert.txt'
EXAMPLES_participants_PATH <- '/home/marta/Documents/erka/EXAMPLES_participants.txt'

  
# # Get vector of IDs 
# docs_collection_basectag_HUIDS = sort(gsub(".txt.bctag", "", list.files(path = docs_collection_basectag_DIR)))
# docs_collection_baseform2_HUIDS = sort(gsub(".txt.bas", "", list.files(path = docs_collection_baseform2_DIR)))
# docs_collection_raw_HUIDS = sort(gsub(".txt", "", list.files(path = docs_collection_raw_DIR)))
# docs_collection_tagged_HUIDS = sort(gsub(".txt.tag", "", list.files(path = docs_collection_tagged_DIR)))

huid_vec = sort(gsub(".txt.bctag", "", list.files(path = docs_collection_basectag_DIR)))
...
statystyki_macka - statystyki_macka.csv
...
EXAMPLES_expert_DATA <- EXAMPLES_expert_DATA[, c('uuid_h2' , 'BULWAROWY')]
...
write.csv(LEARN_SAMPLE_RES, file = "data_sample_with_response_NEW.csv", fileEncoding = "UTF-8")
...
names(LEARN_SAMPLE_RES)


LEARN_SAMPLE_RES$isbulw <- factor(ifelse(LEARN_SAMPLE_RES$isbulw == 1, "Tr","Fa" ))



rfFit <- train(factor(isbulw) ~ .-huid, data = LEARN_SAMPLE_RES, 
               method='rf',
...
rfFit
df_1$totalcnt <- 1:47539
names(df_1) <- gsub("_", "", names(df_1))
wynik <- predict(rfFit,newdata = df_1)
(df_1$trzykropekFRAC) <- as.numeric(as.character(df_1$trzykropekFRAC))
